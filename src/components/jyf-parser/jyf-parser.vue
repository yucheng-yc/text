...
<script>
  ...
  function emojiHandler(html) {
    const reg = new RegExp('"/static/tinymce4.7.5/plugins', 'g');
    return html.replace(reg, '"https://scrm-pic.cdn.buerkongjian.com/static/tinymce4.7.5/plugins');
  }
	var cache = {},
	...
	export default {
		...
		watch: {
			html(html) {
				this.setContent(emojiHandler(html));
			}
		},
		mounted() {
			// 图片数组
			...
			this.$nextTick(() => {
        // #endif
				if (this.html) this.setContent(emojiHandler(this.html));
				// #ifdef APP-PLUS-NVUE
			})
			// #endif
		},
      ...
      this.imgList.each(src => {
		...
		methods: {
          ...
          //   img.src = 'https://scrm-pic.cdn.buerkongjian.com' + src
          // }
          img.src = 'https://scrm-pic.cdn.buerkongjian.com' + src
